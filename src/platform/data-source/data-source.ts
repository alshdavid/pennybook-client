...
import type { AccountDetail, AccountId, CreateAccountOptions, TransactionDetail, TransactionId } from '../../../worker/platform/data-source/index.ts'
...
export class DataSource {
  ...
  constructor() {
    ...
    pbWorker.subscribe<Record<string, AccountDetail>>('accounts', (accounts) => this.accounts.next(accounts))
    pbWorker.subscribe<Record<string, TransactionDetail>>('transactions', (transactions) => this.transactions.next(transactions))
  ...
  async addTransactions(...transaction: Array<Omit<TransactionDetail, 'transactionId'>>): Promise<void> {
    await pbWorker.send('transactions/add', transaction)
  }

  async updateTransaction(transactionId: TransactionId, options: TransactionDetail): Promise<void> {
    await pbWorker.send('transactions/update', [
      transactionId,
      options,
    ])
  }
