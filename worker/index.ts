...
import { AccountId, TransactionDetail, TransactionId, type CreateAccountOptions } from "./platform/data-source/index.ts";
import { ConnectionManager } from "./platform/shared-worker/connection-manager.ts";
import { patchConsole, globalErrorHandler } from "./platform/shared-worker/patches.ts";
...
clients.onConnect(conn => {
  ...
  conn.subscribe<CreateAccountOptions>('accounts/create', async (payload, res) => {
  ...
  conn.subscribe<Array<TransactionDetail>>('transactions/add', async (payload, res) => {
  ...
  conn.subscribe<[TransactionId, TransactionDetail], void>('transactions/update', async (payload, res) => {
