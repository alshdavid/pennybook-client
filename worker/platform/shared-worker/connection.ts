export class Connection extends EventTarget {
  #port: MessagePort;
  #id: string;
  #listeners: Map<string, Set<(v: any, res: { res: (v: any) => void, err: (e: any) => void }) => (any | Promise<any>)>>
  ...
  #defaultListener = ({ data }: { data: { id?: string, action: string, payload: unknown } }) => {
  ...
  // @ts-expect-error
  addEventListener(message: string, cb: (event: { data: { id?: string, action: string, payload: unknown } }) => any, options?: { once?: boolean })
  // @ts-expect-error
  addEventListener(
    ...args: any
  ) {
  ...
  subscribe<T, R = any>(action: string, callback: (payload: T, res: { res: (v: any) => void, err: (e: any) => void }) => any): () => void {
    const listeners = this.#listeners.get(action) || new Set();
